...
public sealed class PacketAwaitable<TPacket> : IPacketAwaitable where TPacket : PlayPacket
{
    private readonly TaskCompletionSource<PlayPacket> _promise = new(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    public void Complete(PlayPacket packet)
    {
        if (packet == null) 
